...
export class CoreTokenMicroServiceListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private coreTokenMicroServiceService: CoreTokenMicroServiceService,
  ...
  DataGetAll(): void {
    ...
    /*filter CLone*/
    this.coreTokenMicroServiceService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.coreTokenMicroServiceService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.coreTokenMicroServiceService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.coreTokenMicroServiceService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
