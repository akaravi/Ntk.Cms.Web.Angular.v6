...
export class CoreTokenActivationListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    /*filter CLone*/
    this.coreTokenActivationService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onActionbuttonDeleteRow(model: CoreTokenActivationModel = this.tableRowSelected): void {
    ...
    this.loading.Start(pName);
          ...
          this.coreTokenActivationService.ServiceDelete(this.tableRowSelected.Id).subscribe(
            (next) => {
              if (next.IsSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.coreTokenActivationService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.coreTokenActivationService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.coreTokenActivationService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
