...
export class SmsLogApiPathListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    this.smsLogApiPathService.ServiceGetAllEditor(filterModel).subscribe({
      next: (ret) => {
    ...
    this.cmsConfirmationDialogService.confirm(title, message)
      .then((confirmed) => {
        if (confirmed) {
          ...
          this.smsLogApiPathService.ServiceDelete(this.tableRowSelected.id).subscribe({
            next: (ret) => {
              if (ret.isSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.smsLogApiPathService.ServiceGetCount(this.filteModelContent).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
    ...
    this.smsLogApiPathService.ServiceGetCount(filterStatist1).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.smsLogApiPathService.ServiceExportFile(model).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
