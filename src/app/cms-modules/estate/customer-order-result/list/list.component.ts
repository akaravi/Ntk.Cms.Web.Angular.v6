...
export class EstateCustomerOrderResultListComponent implements OnInit, OnDestroy {
  ...
  requestLinkProperty = '';
  constructor(
    private estateCustomerOrderResultService: EstateCustomerOrderResultService,
  ...
  DataGetAll(): void {
    ...
    this.estateCustomerOrderResultService.ServiceGetAll(filterModel).subscribe({
      next: (ret) => {
    ...
    this.cmsConfirmationDialogService.confirm(title, message)
      .then((confirmed) => {
        if (confirmed) {
          ...
          this.estateCustomerOrderResultService.ServiceDelete(this.tableRowSelected.id).subscribe({
            next: (ret) => {
              if (ret.isSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.estateCustomerOrderResultService.ServiceGetCount(this.filteModelContent).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
    ...
    this.estateCustomerOrderResultService.ServiceGetCount(filterStatist1).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.estateCustomerOrderResultService.ServiceExportFile(model).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
