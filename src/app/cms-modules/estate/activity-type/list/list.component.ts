...
export class EstateActivityTypeListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    /*filter CLone*/
    this.estateActivityTypeService.ServiceGetAllEditor(filterModel).subscribe({
      next: (ret) => {
    ...
    this.cmsConfirmationDialogService.confirm(title, message)
      .then((confirmed) => {
        if (confirmed) {
          ...
          this.estateActivityTypeService.ServiceDelete(this.tableRowSelected.id).subscribe({
            next: (ret) => {
              if (ret.isSuccess) {
  ...
  onActionbuttonContentDetailList(model: EstateActivityTypeModel = this.tableRowSelected): void {
    if (!model || !model.id || model.id.length === 0) {
  ...
  onActionbuttonContentList(model: EstateActivityTypeModel = this.tableRowSelected): void {
    if (!model || !model.id || model.id.length === 0) {
      const message = this.translate.instant('MESSAGE.no_row_selected_to_display');
      this.cmsToastrService.typeErrorSelected(message);
      return;
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.estateActivityTypeService.ServiceGetCount(this.filteModelContent).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
    ...
    this.estateActivityTypeService.ServiceGetCount(filterStatist1).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.estateActivityTypeService.ServiceExportFile(model).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
