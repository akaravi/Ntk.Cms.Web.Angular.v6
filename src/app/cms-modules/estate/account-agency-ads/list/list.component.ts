...
@Component({
  selector: 'app-estate-accountagencyads-list',
...
export class EstateAccountAgencyAdsListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    /*filter CLone*/
    this.estateAccountAgencyAdsService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
    ...
    this.cmsConfirmationDialogService.confirm(title, message)
      .then((confirmed) => {
        if (confirmed) {
          ...
          this.estateAccountAgencyAdsService.ServiceDelete(this.tableRowSelected.Id).subscribe(
            (next) => {
              if (next.IsSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.estateAccountAgencyAdsService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.estateAccountAgencyAdsService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.estateAccountAgencyAdsService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
