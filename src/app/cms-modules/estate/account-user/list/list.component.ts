...
export class EstateAccountUserListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    /*filter CLone*/
    this.estateAccountUserService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
  ...
  onActionbuttonDeleteRow(model: EstateAccountUserModel = this.tableRowSelected): void {
    ...
    this.loading.Start(pName);
          ...
          this.estateAccountUserService.ServiceDelete(this.tableRowSelected.Id).subscribe(
            (next) => {
              if (next.IsSuccess) {
  ...
  onActionbuttonContentList(model: EstateAccountUserModel = this.tableRowSelected): void {
    if (!model || !model.Id || model.Id.length === 0) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.estateAccountUserService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.estateAccountUserService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.estateAccountUserService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
