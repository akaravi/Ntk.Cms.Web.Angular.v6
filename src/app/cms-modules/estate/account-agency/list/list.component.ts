...
@Component({
  selector: 'app-estate-accountagency-list',
...
export class EstateAccountAgencyListComponent implements OnInit, OnDestroy {
  ...
  onActionbuttonDeleteRow(model: EstateAccountAgencyModel = this.tableRowSelected): void {
    ...
    this.loading.Start(pName);
          ...
          this.estateAccountAgencyService.ServiceDelete(this.tableRowSelected.Id).subscribe(
            (next) => {
              if (next.IsSuccess) {
  ...
  onActionbuttonContentList(model: EstateAccountAgencyModel = this.tableRowSelected): void {
    if (!model || !model.Id || model.Id.length === 0) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.estateAccountAgencyService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.estateAccountAgencyService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.estateAccountAgencyService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
