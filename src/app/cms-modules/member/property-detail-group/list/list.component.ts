...
export class MemberPropertyDetailGroupListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    /*filter CLone*/
    this.memberPropertyDetailGroupService.ServiceGetAllEditor(filterModel).subscribe({
      next: (ret) => {
  ...
  //   }
  //   this.memberPropertyDetailGroupService.ServiceEditStep(model).subscribe({
  //     next: (ret) => {
  //       if (ret.isSuccess) {
    ...
    this.cmsConfirmationDialogService.confirm(title, message)
      .then((confirmed) => {
        if (confirmed) {
          ...
          this.memberPropertyDetailGroupService.ServiceDelete(this.tableRowSelected.id).subscribe({
            next: (ret) => {
              if (ret.isSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.memberPropertyDetailGroupService.ServiceGetCount(this.filteModelContent).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
    ...
    this.memberPropertyDetailGroupService.ServiceGetCount(filterStatist1).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.memberPropertyDetailGroupService.ServiceExportFile(model).subscribe({
      next: (ret) => {
        if (ret.isSuccess) {
