...
import {
  ...
  EnumRecordStatus,
  DataFieldInfoModel,
  CoreModuleSaleHeaderGroupModel} from 'ntk-cms-api';
...
export class CoreModuleSaleHeaderListComponent implements OnInit, OnDestroy {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.coreModuleSaleHeaderService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.coreModuleSaleHeaderService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.coreModuleSaleHeaderService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
