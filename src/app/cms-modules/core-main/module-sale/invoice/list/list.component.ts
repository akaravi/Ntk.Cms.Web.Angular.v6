...
export class CoreModuleSaleInvoiceListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    this.coreModuleSaleInvoiceService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onActionbuttonDeleteRow(model: CoreModuleSaleInvoiceModel = this.tableRowSelected): void {
    ...
    this.loading.Start(pName);
          ...
          this.coreModuleSaleInvoiceService.ServiceDelete(this.tableRowSelected.Id).subscribe(
            (next) => {
              if (next.IsSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.coreModuleSaleInvoiceService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.coreModuleSaleInvoiceService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.coreModuleSaleInvoiceService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
