...
export class CoreLocationListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    this.coreLocationService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.coreLocationService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.coreLocationService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
