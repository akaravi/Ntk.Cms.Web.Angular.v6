...
export class ApiTelegramLogOutputListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    /*filter CLone*/
    this.apiTelegramLogOutputService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onActionbuttonDeleteRow(model: ApiTelegramLogOutputModel = this.tableRowSelected): void {
    ...
    //       this.apiTelegramLogOutputService.ServiceDelete(this.tableRowSelected.Id).subscribe(
    //         (next) => {
    //           if (next.IsSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.apiTelegramLogOutputService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.apiTelegramLogOutputService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.apiTelegramLogOutputService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onActionbuttonLinkTo(model: ApiTelegramLogOutputModel = this.tableRowSelected): void {
    ...
    // this.apiTelegramLogOutputService      .ServiceGetOneById(this.tableRowSelected.Id)
    //   .subscribe(
    //     (next) => {
    //       if (next.IsSuccess) {
    //         //open popup
