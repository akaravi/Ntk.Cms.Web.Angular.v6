...
export class ApiTelegramUploadedFileListComponent implements OnInit, OnDestroy {
  constructor(
    private apiTelegramUploadedFileService: ApiTelegramUploadedFileService,
  ...
  DataGetAll(): void {
    ...
    /*filter CLone*/
    this.apiTelegramUploadedFileService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.cmsConfirmationDialogService.confirm(title, message)
      .then((confirmed) => {
        if (confirmed) {
          ...
          this.apiTelegramUploadedFileService.ServiceDelete(this.tableRowSelected.Id).subscribe(
            (next) => {
              if (next.IsSuccess) {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.apiTelegramUploadedFileService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.apiTelegramUploadedFileService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.apiTelegramUploadedFileService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
