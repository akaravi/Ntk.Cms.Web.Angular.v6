...
export class LinkManagementTargetListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    this.linkManagementTargetService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.linkManagementTargetService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.linkManagementTargetService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.linkManagementTargetService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
