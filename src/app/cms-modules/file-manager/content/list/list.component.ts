...
export class FileContentListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    this.fileContentService.ServiceGetAllEditor(filterModel).subscribe(
      (next) => {
  ...
  onActionbuttonStatist(): void {
    ...
    statist.set('All', 0);
    this.fileContentService.ServiceGetCount(this.filteModelContent).subscribe(
      (next) => {
        if (next.IsSuccess) {
    ...
    this.fileContentService.ServiceGetCount(filterStatist1).subscribe(
      (next) => {
        if (next.IsSuccess) {
  ...
  onSubmitOptionExport(model: FilterModel): void {
    ...
    this.fileContentService.ServiceExportFile(model).subscribe(
      (next) => {
        if (next.IsSuccess) {
